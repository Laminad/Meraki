        ...
        with open(output_file, "w") as ofile:
            for serial in serial_array:
                ...
                try:
                    ...
                    print(f"{dt.now()} INFO: No l3 firewall rules configured on {network['name']}")
                try:
                    ...
                    print(f"{dt.now()} INFO: No l7 Firewall rules configured on {network['name']}")
                try:
                    vpn_bgp_settings = m.appliance.getNetworkApplianceVpnBgp(network_id)
                    device_config["vpn_bgp_settings"] = vpn_bgp_settings
                except:
                    print(f"{dt.now()} INFO: No bgp configuration applied on {network['name']}")
                try:
                    ...
                    print(f"{dt.now()} INFO: No site to site vpn configuration applied on {network['name']}")
                try:
                    ...
                    print(f"{dt.now()} INFO: No vpn firewall rules configured on {network['name']}")
                try:
                    ...
                    print(f"{dt.now()} INFO: No traffic shaping uplink policies configured on {network['name']}")
                try:
                    ...
                    print(f"{dt.now()} INFO: No traffic shaping bandwidth limited configured on {network['name']}")
